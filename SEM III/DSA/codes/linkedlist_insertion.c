//information: p is the pointer variable which traverses and points each node, new_node is the new node that i create ,head is the first node of the linkedlist
...
node* insert_at_beginning(node* head, int value_to_insert){
    ...
    new_node -> data = value_to_insert; //naya node ko data ma insert grna khojeko value haldine
    new_node -> next = head; //ani yo node le chai head lai point grxa kinaki yo beginning ma insert vayesi head yo paxiauxa
    return new_node; //returns this new formed node
...
struct node* insert_at_end(struct node* head, int value_to_insert){
    ...
    struct node* new_node = (node*)malloc(sizeof(node)); //make new node
...
struct node* insert_after_node(node* head, node* nodename , int value_to_insert){
    node* new_node = (node*)malloc(sizeof(node));
     new_node-> data = value_to_insert;
     new_node->next = nodename->next;
     nodename->next = new_node;
    return head;
}
